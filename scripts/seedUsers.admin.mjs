/**
 * seedUsers.admin.mjs — uses Firebase Admin SDK (bypasses Firestore rules)
 * Run:  node scripts/seedUsers.admin.mjs
 */
...
// Dynamic import for firebase-admin from backend/node_modules
...
const USERS = [
  ...
  },
  {
    ...
    age: 29,
    city: 'Bengaluru',
    bio: 'SWE at FAANG. Weekend trekker 🏔️ Done Kedarkantha, Hampta Pass, Kuari Pass. Next: Chadar Trek.',
  ...
  },
  {
    ...
    age: 30,
    city: 'Kolkata',
    bio: 'Journalist. Bookworm, street photographer, terrible chess player trying to improve ♟️',
  ...
  },
  {
    ...
    age: 24,
    city: 'Bengaluru',
    bio: 'Marketing at a D2C brand. Brunch enthusiast 🥞 Weekend pottery classes and obsessed with plants 🌿',
  ...
  },
  {
    ...
    age: 27,
    city: 'Chennai',
    bio: 'Carnatic musician + software developer. Jam sessions, filter coffee and late night debugging ☕',
  ...
  },
  {
    ...
    age: 23,
    city: 'Bengaluru',
    bio: 'Comedian + content creator 🎭 3M views on YouTube but still cannot figure out how to cook rice.',
...
async function seed() {
  console.log(`Seeding ${USERS.length} demo users…\n`);
  const now = Date.now();
  ...
  for (let i = 0; i < USERS.length; i++) {
    ...
    const docData = {
      ...
      createdAt: now - i * 3_600_000,   // stagger so orderBy createdAt works
    };
    await db.collection('users').doc(u.uid).set(docData, { merge: true });
    console.log(`  ✓ ${u.name} (${u.city})`);
  }

  console.log('\n✅ Done! Pull-to-refresh the Discovery screen to see all users.');
  process.exit(0);
}

seed().catch((err) => { console.error('Seed failed:', err.message ?? err); process.exit(1); });
